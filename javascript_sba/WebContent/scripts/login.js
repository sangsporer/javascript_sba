...
function main_validator(e) {
    ...
    e.preventDefault();
    ele.innerHTML = "";
    if (check_email(e.target[0].value)
        && check_psw (e.target[1].value)){
            element.submit();
        }
}
...
function check_psw (psw) {
    var rep = new RegExp(/^.*(?=.{6,})(?=.*d)(?=.*[A-Z])(?=.*[a-z])(?=.*[!@#$%^&*? ]).*$/); 
    var message = "";
    var someWrong = false;
    console.log(rep.test(psw))
    if(!rep.test(psw)){
        message += "The password must contain one lowercase letter one uppercase letter, one number, one unique character such as !@#$%^&? and be at least 6 characters long."
        someWrong = true;
    }
    if(someWrong){
        // Create a <p> element
